...
# Class to manage execution evidence: reports and failure screenshots
class EvidenceHandler
  ...
  class << self
    ...
    def init_paths(report_path = nil, evidence_path = nil)
      rpath = "results/reports/#{Time.now.strftime('%F')}"
      epath = "results/evidence/#{ENVIRON}/#{Time.now.strftime('%F')}/#{Time.now.strftime('%H%M%S')}"
    ...
    # saves screenshot for the specified driver and returns path of screenshot
    ...
    def save(scenario)
      ...
      page.save_screenshot(path_to_failure.to_s)
    ...
    def save_step(step)
      ...
      page.save_screenshot(path_to_ss.to_s)
    end
    ...
    # saves screenshot for the specified driver and given name
    # @param [selenium_webdriver_object] driver the current driver object for execution
    ...
    def save_evidence(driver, name)
      ...
      driver.save_screenshot(@path_to_evidence.to_s)
    end
